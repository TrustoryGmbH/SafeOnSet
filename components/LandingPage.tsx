...
import { ArrowRight, Shield, BarChart3, Lock, LogIn, Phone, CheckCircle, Globe, X, Beaker, ClipboardCheck, Play, Users, Zap, Search } from 'lucide-react';
...
const LandingPage: React.FC<LandingPageProps> = ({ lang, setLang, onLoginClick, onTestAccess, onEnterTestCode }) => {
  ...
  const [activeModal, setActiveModal] = useState<'none' | 'privacy' | 'terms' | 'imprint' | 'test-options'>('none');
    ...
    <div className={`min-h-screen bg-[#0f172a] text-white flex flex-col relative overflow-x-hidden ${lang === 'ar' ? 'font-tajawal' : 'font-sans'}`} dir={t.dir}>
        ...
        <div className="flex items-center gap-6">
            <div className="hidden md:flex gap-1 bg-white/5 p-1 rounded-full border border-white/5 backdrop-blur-sm">
                ...
                <button
                    key={l}
                    onClick={() => setLang(l)}
                    className={`text-xl px-2 py-1 rounded-full transition-all ${lang === l ? 'bg-white/10 text-white' : 'text-slate-500 hover:text-slate-300'}`}
                >
                    {l === 'en' ? '🇬🇧' : l === 'de' ? '🇩🇪' : '🇸🇦'}
                </button>
                ))}
            ...
            <button 
                ...
                className="flex items-center gap-2 bg-white/10 hover:bg-white/20 text-white px-5 py-2.5 rounded-full text-sm font-bold transition-all border border-white/5 backdrop-blur-md group"
            >
                <LogIn size={16} />
                <span>Login</span>
            </button>
      ...
      <main className="relative z-10 flex flex-col items-center justify-center px-6 text-center max-w-5xl mx-auto pt-20 pb-32">
         ...
         <div className="flex flex-col sm:flex-row items-center gap-4 mb-20">
             <button 
                onClick={onLoginClick}
                className="h-16 px-10 bg-blue-600 hover:bg-blue-500 text-white text-xl font-black rounded-2xl shadow-[0_10px_40px_rgba(37,99,235,0.4)] transition-all transform hover:scale-105 active:scale-95 flex items-center gap-3 group"
             >
                {t.landCTA} <ArrowRight className="group-hover:translate-x-1 transition-transform" />
             </button>
             
             <button 
                onClick={() => setActiveModal('test-options')}
      ...
      <section className="relative z-10 py-32">
        <div className="max-w-7xl mx-auto px-8">
           <h2 className="text-4xl md:text-6xl font-black text-center mb-20 uppercase tracking-tight italic">
      ...
      {/* Modals */}
      {activeModal !== 'none' && (
          <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-black/90 backdrop-blur-md" onClick={() => { setActiveModal('none'); setShowCodeInput(false); }}>
              <div 
                className="bg-[#0f172a] border border-white/10 rounded-[32px] shadow-2xl max-w-md w-full overflow-hidden"
                onClick={(e) => e.stopPropagation()}
              >
                 <div className="flex items-center justify-between p-6 border-b border-white/10">
                    <h2 className="text-xl font-black text-white uppercase tracking-tight">
                        {activeModal === 'test-options' ? (showCodeInput ? t.enterTestCode : t.testAccess) : 'Information'}
                    </h2>
                    <button onClick={() => { setActiveModal('none'); setShowCodeInput(false); }} className="text-slate-400 hover:text-white transition-colors"><X size={24} /></button>
                 </div>
                 
                 <div className="p-8">
                    {activeModal === 'test-options' ? (
                        showCodeInput ? (
                            <div className="space-y-6 animate-in slide-in-from-right-4 duration-300">
                                <div className="text-center mb-6">
                                    <Beaker size={48} className="text-blue-500 mx-auto mb-4" />
                                    <p className="text-slate-400 text-sm">Enter the code provided by the production board.</p>
                                </div>
                                <input 
                                    type="text" 
                                    value={testCode}
                                    ...
                                    placeholder="XXXXX"
                                    className="w-full py-5 text-center bg-slate-900 border border-blue-500/30 rounded-2xl text-white text-3xl font-black tracking-[0.5em] outline-none focus:border-blue-500"
                                    maxLength={5}
                                    autoFocus
                                />
                                <button 
                                    onClick={() => onEnterTestCode(testCode)}
                                    className="w-full py-4 bg-blue-600 text-white font-black rounded-xl uppercase tracking-widest shadow-lg shadow-blue-900/50 transition-all hover:bg-blue-500"
                                >
                                    Confirm Code
                                </button>
                                <button onClick={() => setShowCodeInput(false)} className="w-full text-slate-500 text-xs font-bold uppercase tracking-widest py-2">Back</button>
                            </div>
                        ) : (
                            <div className="space-y-4">
                                <button 
                                    onClick={() => setShowCodeInput(true)}
                                    className="w-full flex items-center gap-4 p-5 bg-blue-600/10 border border-blue-500/20 rounded-2xl hover:bg-blue-600/20 transition-all text-left group"
                                >
                                    ...
                                    <div>
                                        ...
                                        <div className="text-slate-400 text-xs mt-1">Direct login via code XPLM2</div>
                                    </div>
                                </button>

                                <button 
                                    onClick={() => { onTestAccess('request'); setActiveModal('none'); }}
                                    className="w-full flex items-center gap-4 p-5 bg-slate-800/40 border border-white/10 rounded-2xl hover:bg-slate-800 transition-all text-left group"
                                >
                                    <div className="w-12 h-12 bg-slate-700 rounded-xl flex items-center justify-center text-slate-300 shrink-0 group-hover:scale-110 transition-transform">
                        ...
                        <div className="text-slate-300 whitespace-pre-line font-medium text-sm leading-relaxed max-h-[400px] overflow-y-auto pr-4">
                 ...
                 <div className="p-6 bg-slate-950/50 flex justify-end">
                    <button onClick={() => { setActiveModal('none'); setShowCodeInput(false); }} className="px-8 py-3 bg-slate-800 text-white text-[10px] font-black rounded-xl uppercase tracking-widest border border-white/5">{t.close}</button>
                 </div>
              </div>
          </div>
      )}
